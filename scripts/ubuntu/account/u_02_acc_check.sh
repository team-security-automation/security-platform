...
EVIDENCE="비밀번호 복잡도 정책이 없으면 무차별 대입 공격에 취약해집니다."
...
if [ -n "$minlen" ] && [ "$minlen" -ge 8 ] 2>/dev/null; then
  ...
  status="취약"; current="minlen 미설정 또는 8 미만(libpam-pwquality 확인)"
fi
print_json "$CHECK_ID" "$CATEGORY" "$status" "$current" "minlen>=8" "$EVIDENCE"