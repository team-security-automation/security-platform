#!/bin/bash
# KISA 2026 U-44 - tftp, talk 서비스 비활성화
...
CHECK_ID="U-44"
CATEGORY="서비스 관리"
EXPECTED_VALUE="tftp/talk/ntalk 서비스 비활성화"
RISK_LEVEL="상"
...
SYSTEMD=$(running_units_matching '(^|[-_.@])(tftp|tftpd|talk|talkd|ntalk)([-_.@]|$)')
INETD=$(active_inetd_lines '(^|[[:space:]])(tftp|talk|ntalk)([[:space:]]|$)')
XINETD=$(active_xinetd_services 'tftp|talk|ntalk')

if [ -n "$SYSTEMD$INETD$XINETD" ]; then
    STATUS="취약"
    CURRENT_VALUE="tftp/talk/ntalk 서비스 활성화"
    EVIDENCE="systemd=${SYSTEMD:-없음}; inetd=${INETD:-없음}; xinetd=${XINETD:-없음}"
else
    STATUS="양호"
    CURRENT_VALUE="tftp/talk/ntalk 서비스 비활성화"
