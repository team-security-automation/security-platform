...
if systemctl is-active --quiet dnf-automatic.timer 2>/dev/null; then
    STATUS="조치불필요"; ACTION="자동 업데이트(dnf-automatic.timer)가 이미 활성 상태"
else
    systemctl enable --now dnf-automatic.timer >/tmp/u64_fix.log 2>&1
    FIX_RC=$?
    if [ $FIX_RC -eq 0 ]; then
        STATUS="조치완료"; ACTION="자동 보안 업데이트(dnf-automatic.timer)를 활성화함. 대기 중인 패치는 'dnf upgrade --refresh' 로 영향도 검토 후 별도 적용 필요"
    else
