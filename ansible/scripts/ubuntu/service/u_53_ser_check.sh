...
elif [ "$VSFTPD_ACTIVE" = "yes" ] && [ -f "/etc/vsftpd.conf" ]; then
    VALUE=$(grep -iE '^\s*ftpd_banner' "/etc/vsftpd.conf" 2>/dev/null)
    CMD_RC=$?
    if [ -n "$VALUE" ]; then
...
elif [ "$PROFTPD_ACTIVE" = "yes" ] && [ -f "/etc/proftpd/proftpd.conf" ]; then
    VALUE=$(grep -iE 'ServerIdent' "/etc/proftpd/proftpd.conf" 2>/dev/null)
    CMD_RC=$?
