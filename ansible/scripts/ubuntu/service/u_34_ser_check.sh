#!/bin/bash
# KISA 2026 U-34 - Finger 서비스 비활성화
# Target: Ubuntu 24/26
...
CHECK_ID="U-34"
CATEGORY="서비스 관리"
EXPECTED_VALUE="Finger 서비스 비활성화"
RISK_LEVEL="상"
...
SYSTEMD=$(running_units_matching '(^|[-_.@])(finger|fingerd)([-_.@]|$)')
INETD=$(active_inetd_lines '(^|[[:space:]])finger([[:space:]]|$)|fingerd')
XINETD=$(active_xinetd_services 'finger|fingerd')

if [ -n "$SYSTEMD$INETD$XINETD" ]; then
    STATUS="취약"
    CURRENT_VALUE="Finger 서비스 활성화"
    EVIDENCE="systemd=${SYSTEMD:-없음}; inetd=${INETD:-없음}; xinetd=${XINETD:-없음}"
else
    STATUS="양호"
    CURRENT_VALUE="Finger 서비스 비활성화"
    EVIDENCE="systemd/inetd/xinetd에서 활성화된 Finger 서비스가 확인되지 않음"
fi
