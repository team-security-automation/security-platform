...
# 실행: sudo bash u_19_file_check.sh
# ============================================================
...
EXPECTED_VALUE="소유자 root, 권한 644 이하"
RISK_LEVEL="상"
...
if [ ! -f "$FILE" ]; then
    ...
    if [ "$OWNER" = "root" ] && [ "$PERM_NUM" -le 644 ]; then
        STATUS="양호"; CURRENT_VALUE="소유자:$OWNER 권한:$PERM"; EVIDENCE="기준(root 소유, 644 이하) 충족"
    else
        STATUS="취약"; CURRENT_VALUE="소유자:$OWNER 권한:$PERM"; EVIDENCE="기준(root 소유, 644 이하) 미충족"
    fi
fi
