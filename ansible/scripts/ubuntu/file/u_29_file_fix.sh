...
FILE="/etc/hosts.lpd"
if [ ! -f "$FILE" ]; then
    STATUS="조치불필요"; ACTION="$FILE 파일이 존재하지 않음(양호 상태)"
else
    ...
    if [ "$OWNER" = "root" ] && [ "$PERM_NUM" -le 600 ]; then
        ...
        FIX_RC=$?
        if [ $FIX_RC -eq 0 ]; then
            STATUS="조치완료"; ACTION="chown root, chmod 600 을 적용함"
        else
